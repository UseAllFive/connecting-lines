...
export default class Block extends Container {
  ...
  onFirstClick() {
    ...
    TweenMax.to(this.imageContainer, .5, {alpha: 1, ease: Power4.easeOut});
    TweenMax.to(this.arrow, .25, {alpha: 1, x: this.arrow.__startPos + 10, delay: .1, ease: Power4.easeOut});
    TweenMax.to(this.link, .25, {alpha: 1, x: this.link.__startPos + 10, delay: .1, ease: Power4.easeOut});
    this.animateImages();
  }
  ...
  onMouseOut() {
    ...
    TweenMax.to(this.imageContainer, .5, {alpha: .5, ease: Power4.easeOut});
    TweenMax.to(this.arrow, .25, {alpha: 0, x: this.arrow.__startPos, ease: Power4.easeOut});
    TweenMax.to(this.link, .25, {alpha: 0, x: this.link.__startPos, ease: Power4.easeOut});
    this.selected = false;
  ...
  hoverImage(animIn = true) {
    this.imageContainer.children.forEach((image) => {
      const x = animIn ? image.__mouseOnPosition.x : image.__originalPosition.x;
      const y = animIn ? image.__mouseOnPosition.y : image.__originalPosition.y;
