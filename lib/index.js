...
class WocViz {
  ...
  init(props) {
    ...
    this.onWhiteSpaceClickEvent = onWhiteSpaceClickEvent || function(){};
  ...
  onHideAllLines(event) {
    if(event.target instanceof Block || IS_MOBILE()) {
      return;
    }
  ...
  addObjects() {
    ...
    for (const blockData of blocks) {
      ...
      block.on('over', (e) => {
        this.callbackRefs['over'](e);
        this.onExhibitionClick(e);
      });
      block.on('clickDot', (obj) => {
        this.callbackRefs['clickDot'](obj);
        this.onDotClickEvent(obj);
      });
  ...
  clean(cb) {
    if(this.containerLines) {
      ...
      TweenMax.killTweensOf(this.containerLines);
      TweenMax.to(this.containerLines, 0.01, {
    ...
    for (const group of Object.keys(groupDots)) {
      ...
      for (let i = 0; i < points.length - 1; i++) {
        ...
        for (let j = 0; j < curvePoints.length; j++) {
          ...
          const time = (0.45 / curvePoints.length) * this.animationTimingMultiplier;
          timeline.add(
