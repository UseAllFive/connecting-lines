...
import { setSize, getSize, IS_MOBILE, setMobile, setData, getData, setMinHeight } from './utils/config';
...
class WocViz {
  ...
  init(props) {
    ...
    setSize({
      ...
      wr: (width || window.innerWidth) / window.devicePixelRatio,
      h: (height || window.innerHeight),
      h2: (height || window.innerHeight) / 2,
      hr: (height || window.innerHeight) / window.devicePixelRatio,
    })
  ...
  addObjects() {
    ...
    for (const blockData of blocks) {
      const block = new Block(blockData, this.maxImageWidth, this.maxImageHeight);
  ...
  calculatePositionBlocks() {
    ...
    for (const row of this.rows) {
      ...
      for (let i = 0; i < row; i++) {
        ...
        const offset = {x: 10, y: IS_MOBILE() ? 15 : 30};
